...
\begin{itemize}
\item{访问非一致代码段}：当目标是非一致代码段时（目标段段描述符的~C flag~为~0~，见图~\ref{code_data_types}），特权级检查要求调用者的~CPL~与目标代码段的~DPL~相等，而且调用者使用的目标代码段段选择子的~RPL~必须小于等于目标代码段的~DPL。
