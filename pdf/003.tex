...
在~x86~体系结构中，和特权级有关的有几个术语：

\begin{itemize}
\item{\textbf{DPL(Descriptor Privilege Level)}}：描述符特权级，定义于段描述符中。它限制了可以访问此段资源的最低特权级。
\item{\textbf{RPL(Requested Privilege Level)}}：请求特权级，定义于段选择子中。它限制了这个选择子可访问资源的最高特权级。
\item{\textbf{CPL(Current Privilege Level)}}：当前特权级，位于代码段寄存器~CS~中。它代表当前执行程序的特权级，通常情况下与当前代码段的描述符特权级相同。当程序跳转到不同特权级的代码段时，CPL~会随之修改。
\end{itemize}

处理机通过将~CPL~与被访问段的~RPL~和~DPL~其中最大者（数值意义上）相比较，来判断程序是否有权访问另外一个段。如果~$CPL>\max{(RPL, DPL)}~$，那么该访问就是不合法的，处理机就会产生一个~\#GP(General Protection Fault，常规保护错误，中断的一种)。
