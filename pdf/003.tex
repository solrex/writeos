...
我们用同样的方法初始化所有页表。由于页表是连续存储的，我们可以转变初始化~1024~个页表为建立~1024*1024~个连续的页表项，由于我们将所有线性地址映射到与其相同的物理地址，所以第一个页的基址为~0，而每个页的大小为~4KB~，故增加~4096~就得到下个页的基址，以此类推，我们就可以建立起~$1024^2$~个页表项，对应于~$10242$~个页。每个页表项的属性（flag）为：存在内存中、用户级别和可读写。
