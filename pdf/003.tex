...
如果您仔细阅读了图~\ref{protected_seg}~，您就会发现图中并未提到~GDT~，而是使用的~Descriptor Table(DT)~。这是因为对于~x86~架构的~CPU~来说，~DT~总共有两个：我们上节介绍过的~GDT~和下面要介绍的~LDT~。这两个描述符表构成了~x86 CPU~段式存储的基础。
